...
```bash
cd /path/to/groq-mcp-mac-agent/code
...
## Moving This Repository Safely

repo 폴더를 다른 경로(예: `~/Documents/...`)로 옮겨도 괜찮습니다. 다만 launchd plist는 이전 경로를 기억하므로, 이동 직후 아래 복구를 1회 실행해야 합니다.

```bash
cd /new/path/to/groq-mcp-mac-agent/code
./scripts/after_repo_move.sh
```

이 스크립트는 다음을 자동 처리합니다.

- 새 경로 기준 editable install 재적용
- `service-install` 재실행으로 launchd plist 경로 갱신
- 서비스/상태 점검 출력

기본 우선순위는 `~/folder-organizer-v2.yml` -> 레포 루트 `folder-organizer-v2.live.yml` -> 샘플 config 순서입니다.
다른 설정 파일을 쓸 때만 `--config`를 직접 지정하세요.

## Korean Numbered Hierarchy Rules

실서비스(`folder-organizer-v2.live.yml`) 기준으로 아래 규칙을 반영했습니다.

- 최상위는 `001_...`, `002_...`, `003_...` 형태의 대주제 폴더를 우선 사용합니다.
- 하위는 `01_...`, `02_...`, `03_...` 형태의 소주제 폴더를 우선 사용합니다.
- 신규 생성 시에도 기존 번호 체계가 있으면 번호를 이어서 생성합니다.

동작 기준(요약):

- watch 파일은 기존 번호 폴더로 먼저 `merge_existing` 시도
- 유사도 + 문서 키워드 힌트 + 기존 폴더 hit(파일 수) 기반으로 대상 선택
- `임시` 성격이 아닌 파일은 `임시` 버킷을 기본 후보에서 제외
- 주기 문서(연/월 토큰 포함)는 소주제명 생성 시 기간 토큰(`YYYY` 또는 `YYYY-MM`)을 함께 반영

네이밍 정책(실서비스/샘플 공통):

- `naming.delimiter`를 `snake-case`로 설정하여 신규 생성 이름은 `_` 기준으로 정규화
- 기존 원본명은 무조건 일괄 변경하지 않고, 이상/잡음 이름 정리 규칙이 발동되는 경우에만 보정
